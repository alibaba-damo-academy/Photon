...
def get_model_tokenizer_photon(*args, **kwargs):
    stage = kwargs["stage"]
    if stage == 'S1':
        from swift.photon.modeling_photon_s1 import Qwen2_5_VLForConditionalGeneration as Photon_S1
        kwargs['automodel_class'] = kwargs['automodel_class'] or Photon_S1
        model, tokenizer = get_model_tokenizer_multimodal(*args, **kwargs)
    elif stage == 'S2':
        from swift.photon.modeling_photon_s2 import Qwen2_5_VLForConditionalGeneration as Photon_S2
        kwargs['automodel_class'] = kwargs['automodel_class'] or Photon_S2
        model, tokenizer = get_model_tokenizer_multimodal(*args, **kwargs)
    else:
        raise ValueError("Model Version Unsupport.")
    if model is not None:
        base_model = model.model if 'AWQ' in model.__class__.__name__ else model
        patch_get_input_embeddings(base_model.visual, 'vision_patch_embed')
...
def get_model_tokenizer_qwen2_5_vl(*args, **kwargs):
    ...
    if stage == 'S1':
        from swift.photon.modeling_photon_s1 import Qwen2_5_VLForConditionalGeneration as Photon_S1
        kwargs['automodel_class'] = kwargs['automodel_class'] or Photon_S1
        model, tokenizer = get_model_tokenizer_photon(*args, **kwargs)
    elif stage == 'S2':
        from swift.photon.modeling_photon_s2 import Qwen2_5_VLForConditionalGeneration as Photon_S2
        kwargs['automodel_class'] = kwargs['automodel_class'] or Photon_S2
        model, tokenizer = get_model_tokenizer_photon(*args, **kwargs)
        ...
        model, tokenizer = get_model_tokenizer_qwen2_vl(*args, **kwargs)
    return model, tokenizer
