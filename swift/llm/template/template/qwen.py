...
from swift.photon.utils import build_its_kwargs
from swift.llm import to_device
from PIL import Image
from qwen_vl_utils import vision_process
import inspect

from swift.photon.utils import VolumeAugmenter
...
class PhotonTemplate(Template):
    ...
    stage = None
    use_volume = False
    volume_augmentation = VolumeAugmenter()
    ...
    def load_volume(self, volume_path):
        ...
        volume = mtf.Resize(spatial_size=[140, 280, 280], mode="trilinear", align_corners=False)(volume)

        volume = mtf.ScaleIntensity(minv=0.0, maxv=1.0)(volume)
        volume = mtf.ToTensor(track_meta=False)(volume)
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            if ext.lower() == '.gz':
                video = self.load_volume(video)
                self.use_volume = True
                video_kwargs = vision_process.FPS
                video_metadata = dict(
                        fps=vision_process.FPS,
                        frames_indices=[i for i in range(video.size(0))],
            ...
            if mm_data:
                if media_type == 'images':
                    ...
                    if hasattr(processor, 'video_processor'):
                        ...
                        processor_func = processor.image_processor
                        kwargs['images'] = None
                    if self.use_volume == True:
                        media_inputs = processor_func(videos=mm_data, return_tensors='pt', do_resize=False, do_rescale=False, do_normalize=False, **kwargs)
                    else:
                        media_inputs = processor_func(videos=mm_data, return_tensors='pt', do_resize=False, **kwargs)
                    media_grid_thw = media_inputs['video_grid_thw']
                    media_token = self.video_token_id
    ...
    def data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        res = super().data_collator(batch=batch, padding_to=padding_to)
        if self.stage == 'S2':
            res.update(build_its_kwargs(res, self.image_token_id, self.video_token_id, self.vision_end_id, self.im_end_id))                
    ...
    def forward_context(self, model, inputs):
        ...
        text_position_ids = inputs['text_position_ids']
        if self.stage == 'S1':
            from swift.photon import modeling_photon_s1 as modeling_module
        elif self.stage == 'S2':
            from swift.photon import modeling_photon_s2 as modeling_module
        else:
            raise ValueError("Forward Context Version Error.")
        return self._patch_flash_attention_forward(modeling_module, text_position_ids)
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        augmentation = None
        if pixel_values_videos is not None and self.is_training and self.data_aug == True:
            pixel_values_videos, augmentation = self.volume_augmentation(pixel_values_videos)
            inputs['pixel_values_videos'] = pixel_values_videos

        if self.is_training and self.data_aug == False:
            augmentation = False
        ...
        if hasattr(base_model.model, 'embed_tokens'):
            ...
            inputs_embeds = base_model.model.language_model.embed_tokens(input_ids)
        inputs_embeds = super()._get_inputs_embeds_hf(inputs_embeds, inputs, model.visual, self.processor, model.config)
        if augmentation is not None:
            return {'inputs_embeds': inputs_embeds, 'augmentation': augmentation}
        else:
            return {'inputs_embeds': inputs_embeds}
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        ...
        for k, v in old_kwargs.items():
            if k in {
                    ...
                    'max_length_q', 'max_length_k', 'cu_seq_lens_q', 'cu_seq_lens_k', 'vision_mask', 'augmentation', 'question_mask'
            } and k not in kwargs:
                kwargs[k] = v
...
class Photon_S1_Template(PhotonTemplate):
    version = 'v2_5'
    norm_bbox = 'none'
    stage = 'S1'
    data_aug = False

class Photon_S2_Template(PhotonTemplate):
    version = 'v2_5'
    norm_bbox = 'none'
    stage = 'S2'
    data_aug = True

register_template(QwenTemplateMeta(MLLMTemplateType.photon_s1, template_cls=Photon_S1_Template))
register_template(QwenTemplateMeta(MLLMTemplateType.photon_s2, template_cls=Photon_S2_Template))
