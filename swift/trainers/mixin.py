...
from swift.photon.mixmodal_batchsampler import VolumeAwareGlobalBatchSampler
...
class SwiftMixin:
    ...
    def _compute_acc(self, outputs, labels, cu_seqlens=None) -> None:
        ...
        ###########################################
        # Photon Modified
        if outputs.get('labels', None) is not None:
    ...
    def get_train_dataloader(self, skip_batches=0):
        ...
        if dataloader is None:
            ...
            if hasattr(train_dataset, '__len__'):
                ...
                #########################################################
                # Photon Modified
                # batch_sampler = VolumeAwareGlobalBatchSampler(
                    # dataset=train_dataset, batch_size=self._train_batch_size, **batch_sampler_params)
                #########################################################
                dataloader_params['worker_init_fn'] = partial(
                    seed_worker, num_workers=self.args.dataloader_num_workers, rank=self.args.process_index)
                if skip_batches > 0:
