...
class Trainer(SwiftMixin, DataLoaderMixin, HfTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if inputs.get('labels') is not None:
            ##################################
            # Photon Modified
            self._compute_acc(outputs, outputs.get('labels', inputs['labels']))
            ##################################
...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ##########################################
        # Photon Modified
        print("------------------------- Modules Require Grads Starts -------------------------")
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if 'atp' in name:
                    print(f"+Name: {name}, Shape: {param.shape}")
        print("------------------------- Modules Require Grads Ends -------------------------")
        ##########################################
    ...
    ##########################################
    # Photon Modified
    def eval_compute_loss(self, model, inputs, labels):
        # compute usual loss via models
        outputs = model(**inputs, labels=labels, use_cache=False)
        loss = outputs.get('loss')
        logits = outputs.get('logits')
        labels = outputs.get('labels', labels)
    ...
    #####################################
    # Photon Modified
    def get_atp_log(self, outputs):
        its_loss = outputs.get("its_loss", None)
        if its_loss is None:
            self.add_logs = {}
            return
        mllm_loss = outputs.get("mllm_loss", None)
        its_loss = self.accelerator.gather_for_metrics(its_loss).mean().item() if its_loss is not None else None
        mllm_loss = self.accelerator.gather_for_metrics(mllm_loss).mean().item() if mllm_loss is not None else None
        self.add_logs.update({
            'its_loss': its_loss,
            'mllm_loss': mllm_loss,
        })
    #####################################
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is None:
            ##################################
            # Photon Modified
            labels = outputs.get('labels', inputs['labels'])
            ##################################
